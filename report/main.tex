...
The grid search contained 72 combination of hyperparameter configurations and model training was automated to iterate through these combinations (detail in Section \ref{implementation_fnn}). The training and validation performance of these models were stored in corresponding directories throughout the grid search. The results were extracted into a dataframe to analyze the effect of each parameter on the model performance.

\subsubsection{Implementation Detail of FNNs} \label{implementation_fnn}
...
\begin{enumerate}
    ...
    \item \textbf{Model Training Performance Visualization} - After the 72 models were trained and stored in the corresponding folder in the \lstinline{ann_model_results} directory, the model hyperparameters and training results can be extracted from the JSON files and stored in a \lstinline{results_df} dataframe. In this section, the team investigated the top 10 hyperparameter configurations that resulted in the highest training accuracy and validation accuracy. These models are summarized in \autoref{tab:ann_performance_top10_training_accuracy} and \autoref{tab:ann_performance_top10_validation_accuracy}. Furthermore, to investigate the effect of each hyperparameter on a macro level, the resulting data frame was grouped according to the hyperparameter and a box plot was drawn to capture the overall performance at each hyperparameter values (e.g., comparing the general performance for epoch of 10, 25, and 50). These results are included in section \ref{ann_model_result_hyperparameter}
\end{enumerate}
